...
class LRA(keras.callbacks.Callback):
    def __init__(self, custom_model, base_model, patience, stop_patience, threshold, factor, dwell, batches, initial_epoch,
                 epochs, ask_epoch):
        ...
        #self.initial_lr = float(tf.keras.backend.get_value(custom_model.optimizer.learning_rate)) # get the initiallearning rate and save it
    ...
    def on_train_end(self, logs=None):
        ...
        self.custom_model.set_weights(self.best_weights)  # set the weights of the model to the best weights
    ...
    def on_epoch_end(self, epoch, logs=None):  # method runs on the end of each epoch
        ...
        lr = float(tf.keras.backend.get_value(self.custom_model.optimizer.learning_rate))  # get the current learning rate
        current_lr = lr
        ...
        if acc < self.threshold:  # if training accuracy is below threshold adjust lr based on training accuracy
            ...
            if acc > self.highest_tracc:  # training accuracy improved in the epoch
                ...
                self.best_weights = self.custom_model.get_weights()  # traing accuracy improved so save the weights
                ...
                if self.count >= self.patience - 1:  # lr should be adjusted
                    ...
                    tf.keras.backend.set_value(self.custom_model.optimizer.lr, lr)  # set the learning rate in the optimizer
                    self.count = 0  # reset the count to 0
                    self.stop_count = self.stop_count + 1  # count the number of consecutive lr adjustments
                    self.count = 0  # reset counter
                    if self.dwell:
                        self.custom_model.set_weights(
            ...
            if v_loss < self.lowest_vloss:  # check if the validation loss improved
                ...
                self.best_weights = self.custom_model.get_weights()  # validation loss improved so save the weights
...
if __name__ == "__main__":
    img_path = r'car_color_dataset/train/beige/00ecfeedea.jpg'
    img = plt.imread(img_path)
    print('Input image shape is ', img.shape)
    plt.axis('off')
    imshow(img)

    sdir = r'car_color_dataset'
    ...
    batches = train_steps
    callbacks = [LRA(custom_model=model, base_model=base_model, patience=patience, stop_patience=stop_patience, threshold=threshold,
    ...
    # ON 1 IMAGE
    fpath = r'car_color_dataset/train/beige/00ecfeedea.jpg'
...
### scp D:\GIT\Licence_plate_identification\requirements.txt a_herasymchuk@192.168.1.100:/home/a_herasymchuk/Car_recognition